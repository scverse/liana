...
- **Breaking: `masked_spearman` now uses midranks.** Ordinal ranks (`argsort().argsort()`) made the score depend on cell order and biased zero-inflated data upward; it now matches `scipy.stats.spearmanr` per neighbourhood, a neighbourhood in which a feature is constant scores 0, and it is faster (one `argsort` instead of two). Toy pin `0.184 -> 0.003`.
- **Breaking: `li.pl.circle` filters row-wise and within the selected cell types.** With `pivot_mode='mean'`, `filter_fn` kept every pair of an interaction once one pair passed, so edge means included pairs the filter rejected (the tutorial drew 10 CD34+ edges where `'counts'` drew 4). Both modes now keep only the rows that pass. `source_labels`/`target_labels` apply before `filter_fn`/`top_n`, so `top_n` ranks among the selected cell types. A pair whose mean is exactly 0 is now drawn instead of being mistaken for an absent pair.
- **Breaking: `li.pl.circle` sizes nodes by in- plus out-degree of the filtered graph**, not out-degree of the unmasked pivot. Node areas move for every call.
...
- **`compute_global_specificity` compared each permuted group against a different group.** The observed statistic was indexed in category order and the permutations lexicographically (`groups_order` was never read), so p-values were wrong for any non-lexicographic `groupby` (49.8% moved on `pbmc68k`). Frames are now reindexed onto one order and unused categories, which used to crash, are dropped.
- **NATMI warns when `spec_weight` cannot be normalised.** With a single source or target label in the run (e.g. one `groupby_pairs` pair) the normaliser equals the mean itself and `spec_weight` is 1 everywhere; this is by design, and is now said.
- **A single missing score no longer NaNs the whole consensus rank.** `rankdata` propagated one `NaN` (e.g. `0/0` in `spec_weight`) into an all-NaN column that was then sorted silently. Missing scores now take the worst observed value in the column's own direction, as `return_all_lrs` rows already did, with a warning naming the columns.
- **`li.ms.nmf` no longer silently runs a full-rank factorization when no elbow is found.** `estimate_elbow` returning `None` became `NMF(n_components=None)`, i.e. `n_features` components with `nmf_rank = None`. It now raises a `ValueError` naming the `k_range` searched.
- **`li.ms.get_factor_scores` no longer cross-joins the metadata when `obs_names` are duplicated.** `obs[obs_keys]` was merged by barcode although `.obsm` is row-aligned to `.obs`; it is now concatenated positionally. Unchanged for unique `obs_names`.
- **`li.pl.circle` ignored `source_labels`/`target_labels` and could truncate or crash on an asymmetric result (#184, #187).** The labels were only applied as a zero-mask on the pivot, so every node still drew and `mask_mode="or"` widened rather than filtered; and a non-square pivot was silently truncated or raised `Columns must match Indices`. The labels now drop rows before the pivot, honouring `mask_mode`, and the pivot is squared over the labels that remain; a label absent from `adata.obs[groupby]` raises. Output moves for any call passing either label set.
- **An explicitly passed `liana_res` is no longer discarded whenever `adata` is also given (#187).** The shared `_get_liana_res` checked `adata.uns` first, which made it impossible to plot one's own frame with `li.pl.circle`. The frame now takes precedence in every plot and getter that accepts both; `li.pl.elbow` follows the same rule per argument.
- **`li.pl.feature_by_group(normalize=False)` no longer claims a colour range the data exceeds.** `set_clim(0, 1)` sat outside the `if normalize:` block; unnormalised panels now autoscale.
...
- `li.rs.get_metalinks` docstring claimed `source` was dropped; the frame has one row per edge and curating database, so a per-edge mean of `mor` is a vote across them.
...
- **`filter_fn` says what it actually selects.** In `li.pl.dotplot`/`li.pl.tileplot` an interaction is kept when the predicate holds for *any* source-target pair and is then plotted for all of them. This is intended and unchanged; the docstring now says so and names the row-wise cases.

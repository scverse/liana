...
- **`li.pl.circle` ignored `source_labels`/`target_labels` and could truncate or crash on an asymmetric result (#184, #187).** The labels were only applied as a zero-mask on the pivot, so every node still drew and `mask_mode="or"` widened rather than filtered; and a non-square pivot was silently truncated or raised `Columns must match Indices`. The labels now drop rows before the pivot, honouring `mask_mode` -- they narrow the interactions after `filter_fn`/`top_n`, so the edge weights are unchanged -- and the pivot is squared over the labels that remain; a label absent from `adata.obs[groupby]` raises. Output moves for any call passing either label set.
...
- **`filter_fn` says what it actually selects.** In `li.pl.dotplot`/`li.pl.tileplot` (and `li.pl.circle` with `pivot_mode='mean'`) an interaction is kept when the predicate holds for *any* source-target pair and is then plotted for all of them. This is intended and unchanged; the docstring now says so and names the row-wise cases.
